...
public class Recorder implements OnCompletionListener, OnErrorListener {
    ...
    public Recorder(Context context) {
        ...
        String SAMPLE_DEFAULT_DIR = SoundRecorderPreferenceActivity.getSavePath(mContext);
    ...
    public float playProgress() {
        if (mPlayer != null) {
        	/*Log.d("SoundRecorder", "progress >> " + 
        			mPlayer.getCurrentPosition() + "/" + 
        			mDuration);*/
        	
            return ((float) mPlayer.getCurrentPosition()) / mDuration;
        }
        return 0.0f;
    }
    ...
    public void startRecording(String name, String extension, long maxFileSize,
        ...
        RecorderService.startRecording(mContext, mSampleFile.getAbsolutePath(), maxFileSize,
    ...
    public void startPlayback(float percentage) {
        if (state() == PLAYING_PAUSED_STATE) {
            ...
            mPlayer.seekTo((int) (percentage * mDuration));
            mPlayer.start();
            ...
            try {
                ...
                //mDuration = mSampleFile.length() / 128 * 8;
                mDuration = mPlayer.getDuration();

                mPlayer.seekTo((int) (percentage * mDuration));
                mPlayer.start();
                
                if (((AudioManager)mContext.getSystemService("audio")).getStreamVolume(3) == 0)
                    Toast.makeText(mContext, "eeeeeee", 1).show();

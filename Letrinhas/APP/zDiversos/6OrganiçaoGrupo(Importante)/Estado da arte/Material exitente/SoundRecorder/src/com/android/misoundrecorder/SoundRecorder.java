...
public class SoundRecorder extends Activity implements Button.OnClickListener,
	...
	private void setLanguage() {
		String lang = SoundRecorderPreferenceActivity.getLocalization(this);
        Configuration config = getResources().getConfiguration();
        if (!lang.equals("") && !lang.equals(config.locale.getLanguage())) {
        	String[] sp = lang.split("([-]+)");
            locale = sp.length == 2 ? new Locale(sp[0], sp[1]) : new Locale(lang);
        	Locale.setDefault(locale);
            config.locale = locale;
            //Log.i(LOG_TAG, "Set Locale: " + locale);
    ...
    private void initInternalState(Intent i) {
        ...
        if (i != null) {
            String s = i.getType();
            if (AUDIO_MP3.equals(s) || AUDIO_OGG.equals(s) || AUDIO_ANY.equals(s) || ANY_ANY.equals(s)) {
            	mRequestedType = s;
                mShowFinishButton = true;
            } else if (s != null) {
                // I only support mp3 format right now
                setResult(RESULT_CANCELED);
                finish();
                return;
            }
        ...
        if (AUDIO_ANY.equals(mRequestedType)) {
            mRequestedType = SoundRecorderPreferenceActivity.getIsOgg(this) ? AUDIO_OGG : AUDIO_MP3;
    ...
    private void startRecording() {
        ...
        if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            int channelConfig = SoundRecorderPreferenceActivity.getMode(this);
            ...
            mRecorder.startRecording(mFileNameEditText.getText().toString(),
            		isOgg ? FILE_EXTENSION_OGG : FILE_EXTENSION_MP3, mMaxFileSize,
                    channelConfig, sampleRate, bitrate, quality, scale, isOgg);
    ...
    protected void onResume() {
        super.onResume();
        String type = SoundRecorderPreferenceActivity.getIsOgg(this) ? AUDIO_OGG : AUDIO_MP3;
        ...
        if (mRecorder.state() == Recorder.RECORDING_STATE) {
        	String preExtension = AUDIO_MP3.equals(mRequestedType) ? 
        			FILE_EXTENSION_MP3
                    : FILE_EXTENSION_OGG;
            ...
            if (!mRecorder.sampleFile().getName().endsWith(preExtension)) {
                ...
                int sampleRate = SoundRecorderPreferenceActivity.getBitrate(this);
                mRemainingTimeCalculator.setBitRate(sampleRate);
            }
        } else {
    ...
    private void showOverwriteConfirmDialogIfConflicts() {
        String fileName = mFileNameEditText.getText().toString()
                + (AUDIO_MP3.equals(mRequestedType) ? FILE_EXTENSION_MP3 : FILE_EXTENSION_OGG);
    ...
    public void onError(int error) {
        ...
        switch (error) {
            ...
            case Recorder.IN_CALL_RECORD_ERROR:
                // update error message to reflect that the recording
                // could not be
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
        	case R.id.menu_fm:	//case R.id.menu_view_list:
	        	saveSample();
	            ...
	            /*intent = new Intent(this, RecordPreviewActivity.class);
	            intent.putExtra("record_preview_type", 1);
	            if (this.mRecorder.sampleFile() != null)
	            	intent.putExtra("record_preview_highlight", this.mRecorder.sampleFile().getName());
	            */

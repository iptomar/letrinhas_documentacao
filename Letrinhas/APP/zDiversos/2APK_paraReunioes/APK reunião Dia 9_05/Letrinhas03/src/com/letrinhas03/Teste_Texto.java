	...
	private void startAvalia() {
		if (modo) { // se está em modo de professor
			...
			if (file.exists()) { // Verifica se já fez uma gravação
				...
				// define a mensagem
				builder.setMessage("Não foi encontrado nenhuma resolução para\n"
						+ "o teste "+((TextView) findViewById(R.id.textCabecalho)).getText()+"\n"
						+ "para o aluno "+ ((TextView) findViewById(R.id.textRodape)).getText()+".\n"
						+ "Assim não poderá avaliar.\n"
						+ "Se perferir pode executar uma agora!");
